...
func TestSignedRuntimeUpdaterActivatesHealthyCandidateAndKeepsOneRollback(t *testing.T) {
	...
	if !attempt.Succeeded() || attempt.Active.Version != "0.7.5" {
		t.Fatalf("attempt=%+v", attempt)
	}
	var current ActiveRuntime
	readJSONFileForTest(t, filepath.Join(fixture.root, "current.json"), &current)
	if current.Version != "0.7.5" || current.RollbackVersion == nil || *current.RollbackVersion != "0.7.4" {
	...
	if data, err := os.ReadFile(filepath.Join(fixture.root, "versions", "0.7.5", ".activation-confirmed")); err != nil || string(data) != "0.7.5\n" {
...
func TestSignedRuntimeUpdaterReplacesOnlyAnUnactivatedInterruptedCandidate(t *testing.T) {
	fixture := newRuntimeUpdateFixture(t)
	orphan := filepath.Join(fixture.root, "versions", "0.7.5")
...
func TestSignedRuntimeUpdaterRollsBackFailedCandidate(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(fixture.root, "versions", "0.7.5")); !os.IsNotExist(err) {
