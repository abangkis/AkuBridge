...
test("AkuBridge has a narrow read-only permission contract", () => {
  ...
  assert.equal(manifest.version, "0.7.3.0");
...
test("AkuBridge recognizes the current LinkedIn feed container", () => {
  ...
  assert.match(contentScript, /source-adapters-v83/);
  ...
  assert.match(contentScript, /captureTuning\.scrollSettleMs \?\? plan\.scrollSettleMs/);
...
test("AkuBridge exposes additive read-only capabilities and structured failures", () => {
  ...
  assert.equal(capabilities.runtimeRevision, "source-adapters-v83");
  assert.equal(capabilities.buildId, "aku-bridge-0.7.3-source-adapters-v83");
  assert.equal(capabilities.contractVersion, "aku-browser.bridge.v2");
  assert.deepEqual(capabilities.adapterVersions, { x: "x-dom-v21", linkedin: "linkedin-dom-v18", facebook: "facebook-dom-v16" });
