...
test("AkuBridge recognizes the current LinkedIn feed container", () => {
  ...
  assert.match(linkedInAdapter, /linkedin-dom-v20/);
  ...
  assert.match(contentScript, /source-adapters-v109/);
...
test("AkuBridge uses LinkedIn's scroll root and one allowlisted fresh-content activation", () => {
  ...
  assert.match(mediaAcquisitionEngine, /media-acquisition-engine-v4/);
  ...
  assert.match(linkedInAdapter, /linkedin-media-acquisition-v2/);
  assert.match(linkedInAdapter, /AKU_LINKEDIN_COLLECT_STRUCTURED_MEDIA/);
  assert.match(mediaAcquisitionEngine, /await strategy\.extractStructuredCandidates/);
  assert.match(worker, /AKU_LINKEDIN_COLLECT_STRUCTURED_MEDIA/);
...
test("AkuBridge exposes additive read-only capabilities and structured failures", () => {
  ...
  const capabilities = createBridgeCapabilities({ version: "0.9.1.0", version_name: "0.9.1", manifest_version: 3 });
  assert.equal(capabilities.extensionVersion, "0.9.1");
  assert.equal(capabilities.runtimeRevision, "source-adapters-v109");
  assert.equal(capabilities.buildId, "aku-bridge-0.9.1-source-adapters-v109");
  ...
  assert.match(worker, /AKU_BROWSER_RECONCILE_SOURCE_ACCESS[\s\S]*reconcileSourceAccessAndRefreshHeartbeat\(\)/);
  assert.match(worker, /async function reconcileSourceAccessAndRefreshHeartbeat\(\)[\s\S]*return state;/);
